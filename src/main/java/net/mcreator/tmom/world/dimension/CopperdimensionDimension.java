...
import net.mcreator.tmom.item.CopperdimensionItem;
...
public class CopperdimensionDimension extends TmimModElements.ModElement {
	@ObjectHolder("tmim:copperdimension_portal")
	public static final CustomPortalBlock portal = null;

	public CopperdimensionDimension(TmimModElements instance) {
		super(instance, 587);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(CopperOreBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("tmim:copperdiome")).getGenerationSettings().getSurfaceBuilder()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("tmim:copperdimension"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("copperdimension_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("copperdimension_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())),
					0, 1).setRegistryName("copperdimension_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new CopperdimensionItem().setRegistryName("copperdimension"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("copperdimension_portal");
		}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("tmim:copperdimension"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("tmim:copperdimension")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == BlockOfCopperBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? BlockOfCopperBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, BlockOfCopperBlock.block.getDefaultState(), 3);
					}
				}
			}
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getFrom() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("tmim:copperdimension"))) {
		...
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("tmim:copperdimension"))) {
