...
public static class Modelenderdragon extends EntityModel<Entity> {
	...
	public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, Entity e) {
		...
		this.rearleg.rotateAngleX = MathHelper.cos(f * 1.0F) * -1.0F * f1;
		this.head.rotateAngleY = f3 / (180F / (float) Math.PI);
		this.head.rotateAngleX = f4 / (180F / (float) Math.PI);
		this.rearleg1.rotateAngleX = MathHelper.cos(f * 1.0F) * 1.0F * f1;
		this.tail.rotateAngleY = f3 / (180F / (float) Math.PI);
		this.tail.rotateAngleX = f4 / (180F / (float) Math.PI);
		this.frontleg.rotateAngleX = MathHelper.cos(f * 0.6662F) * f1;
		this.frontfoot.rotateAngleX = MathHelper.cos(f * 0.6662F + (float) Math.PI) * f1;
		this.wing1.rotateAngleX = MathHelper.cos(f * 0.6662F + (float) Math.PI) * f1;
		this.wing.rotateAngleX = MathHelper.cos(f * 0.6662F) * f1;
	}
}